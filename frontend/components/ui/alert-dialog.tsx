...
function AlertDialogOverlay({ className, ...props }: AlertDialogPrimitive.Backdrop.Props) {
  return (
    <AlertDialogPrimitive.Backdrop
      ...
      className={cn(
        "data-open:animate-in data-open:fade-in-0 data-closed:animate-out data-closed:fade-out-0 motion-reduce:animate-none motion-reduce:transition-none fixed inset-0 isolate z-50 bg-black/40 duration-100 supports-backdrop-filter:backdrop-blur-xs",
        className
      )}
      {...props}
    />
...
function AlertDialogFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex flex-col-reverse gap-2 group-data-[size=sm]/alert-dialog-content:grid group-data-[size=sm]/alert-dialog-content:grid-cols-2 sm:flex-row sm:justify-end sm:gap-2 [&_[data-slot=button]]:w-full sm:[&_[data-slot=button]]:w-auto [&_[data-slot=alert-dialog-action]]:w-full sm:[&_[data-slot=alert-dialog-action]]:w-auto [&_[data-slot=alert-dialog-cancel]]:w-full sm:[&_[data-slot=alert-dialog-cancel]]:w-auto",
        className
      )}
      {...props}
    />
...
function AlertDialogCancel({
  ...
  return (
    <AlertDialogPrimitive.Close
      ...
      render={<Button variant={variant} size={size} className="h-10 sm:h-9" />}
      {...props}
    />
  );
}
