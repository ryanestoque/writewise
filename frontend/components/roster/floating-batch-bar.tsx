  ...
  return (
    <TooltipProvider delay={200}>
      <aside
        ...
        className="fixed bottom-[max(1rem,calc(env(safe-area-inset-bottom)+0.5rem))] sm:bottom-6 left-1/2 -translate-x-1/2 z-40 w-[calc(100%-1.25rem)] sm:w-auto max-w-xl animate-in fade-in slide-in-from-bottom-4 motion-reduce:animate-none motion-reduce:transition-none duration-200 select-none"
      >
        <span className="sr-only" aria-live="polite" aria-atomic="true">
          {selectedCount} {selectedCount === 1 ? "student" : "students"} selected of {totalCount}. Batch actions available.
        </span>

        <div className="bg-surface/95 dark:bg-card/95 backdrop-blur-md border border-border/80 dark:border-border/60 shadow-[0_8px_30px_rgb(0,0,0,0.12)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.4)] ring-1 ring-black/5 dark:ring-white/10 rounded-2xl sm:rounded-full p-1.5 sm:px-3.5 sm:py-2 flex items-center justify-between sm:justify-start gap-1.5 sm:gap-2.5 text-foreground">
          {/* Left: Count Badge & Context */}
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0 min-w-0">
            <Badge className="bg-brand-700 hover:bg-brand-800 dark:bg-primary dark:hover:bg-primary/90 text-white font-semibold text-xs px-2.5 py-1 rounded-full shadow-2xs shrink-0 flex items-center gap-1 transition-colors">
            ...
            {onSelectAll && !allSelected && totalCount > selectedCount && (
              <button
                type="button"
                onClick={onSelectAll}
                className="hidden lg:inline-flex items-center gap-1 text-[11px] text-primary hover:text-brand-700 dark:text-primary dark:hover:text-primary/80 font-medium ml-0.5 hover:underline underline-offset-2 transition-colors cursor-pointer"
              ...
              <TooltipTrigger
                render={
                  <Button
                    ...
                    disabled={isRemoving}
                    className="h-8 sm:h-8 px-2 sm:px-3 text-xs font-medium border-border/80 hover:bg-muted/80 hover:text-foreground active:scale-[0.97] transition-all shrink-0 rounded-xl sm:rounded-lg"
                  ...
                  >
                    ...
                    <span className="sm:hidden hidden min-[390px]:inline">Move</span>
                  </Button>
                }
              />
              ...
              <TooltipTrigger
                render={
                  ...
                  >
                    ...
                    <span className="sm:hidden hidden min-[390px]:inline">Export</span>
                  </Button>
                }
              />
              <TooltipContent side="top" sideOffset={6} className="text-xs font-normal">
              ...
              <TooltipTrigger
                render={
                  <Button
                    ...
                    onClick={onRemove}
                    disabled={isRemoving}
                    className="h-8 sm:h-8 px-2 sm:px-3 text-xs font-medium bg-destructive/10 text-destructive hover:bg-destructive hover:text-white border border-destructive/20 active:scale-[0.97] transition-all shrink-0 rounded-xl sm:rounded-lg"
                  ...
                  >
                    ...
                    <span className="sm:hidden hidden min-[390px]:inline">Remove</span>
                  </Button>
                }
              />
              <TooltipContent side="top" sideOffset={6} className="text-xs font-normal">
